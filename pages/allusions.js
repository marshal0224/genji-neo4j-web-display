...
export async function getServerSideProps () {
    ...
    const resHonka = await session1.readTransaction(tx => tx.run(getHonka))
    const resPnum = await session2.readTransaction(tx => tx.run(getPnum))
    const resPoemHonka = await session3.readTransaction(tx => tx.run(getPoemHonka))
    const resPoet = await session4.readTransaction(tx => tx.run(getPoet))
    const resPoetEdge = await session5.readTransaction(tx => tx.run(getHonkaPoet))
    const resSrc = await session6.readTransaction(tx => tx.run(getSource))
    const resSourceEdge = await session7.readTransaction(tx => tx.run(getHonkaSource))
    const resTrans = await session8.readTransaction(tx => tx.run(getTrans))
    let ans = []
    let max = 0
    let key = 0
    ...
    for (let i = 0; i < ans.length; i ++) {
        if (ans[i].translations === undefined) {
            delete ans[i].translations
        }
    }
    let init_src = {}
    let init_order = {}
    ...
    return {
        props: {
            initTranslator: translators,
            initData: ans,
            initMaxID: max, 
            initEditSource: init_src,
            initEditOrder: init_order,
            initPnum: ls,
            initAllusion: links,
            initPoet: poets,
            initSource: sources,
...
export default function AllusionTable( props ) {
    const [pnum, setPnum] = useState(props.initPnum)
    ...
    const [poet, setPoet] = useState(props.initPoet)
    const [translators, setTranslators] = useState(props.initTranslator)
    const [source, setSource] = useState(props.initSource)
    const [auth, setAuth] = useState(false)
    const [usr, setUsr] = useState('')
    const [pwd, setPwd] = useState('')
    const [allusion, setAllusion] = useState(props.initAllusion)
    const [maxID, setMaxID] = useState(props.initMaxID)
    ...
    const [editSource, setEditSource] = useState(props.initEditSource)
    const [editOrder, setEditOrder] = useState(props.initEditOrder)
                                ...
                                <>
                                    <Link 
                                        ...
                                        href={{
                                            pathname: `/poems`,
                                            query: {
                                                chapter: parseInt(e[0].substring(0, 2)),
                                                number: parseInt(e[0].substring(4, 6)),
                                            }
                                        }}
                                        target="_blank"
